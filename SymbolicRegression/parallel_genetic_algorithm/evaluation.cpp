...
double calcDiversity(vector<Tree> pop){
    ...
    double diversity = (1.0 - total * normalize_factor) * 100.0;
    //
    */
    int num = 500;
    if (pop.size() < 500){
    ...
    for(int i=0; i<num/2; i++){
        sum_distance += calcDistance(pop[i].nodes, pop[i+num/2].nodes);
    }
    double diversity = (double)sum_distance / ((double)num/2.0);
