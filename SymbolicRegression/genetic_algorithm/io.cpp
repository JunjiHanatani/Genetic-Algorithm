...
// Output
int const DIST_OUT_FREQ = NGEN/100;
int const PATH_OUT_FREQ = NGEN;
std::ofstream ofs_history("./results/history.csv");
std::ofstream ofs_distribution("./results/distribution.csv");
std::ofstream ofs_bestpath("./results/bestpath.csv");
std::ofstream ofs_solution("./results/solution.csv");
...
void output(vector<Tree> pop, bool verbose){
    ...
    double best_mae = best_ind.mae;
    //double worst_fitness = pop[N_POP*NUM_PARALLEL-1].fitness;
    ...
    double diversity = calcDiversity(pop);

    ofs_history << g << ", " << best_mae << ", " << diversity << endl;
    ...
    if (g % DIST_OUT_FREQ == 0){
        for(int i=0; i<N_POP; i++){
            ofs_distribution << g << ", " << pop[i].mae << endl;
        }
    }
    ...
    string func_name = showFunction(best_ind, true);
    ofs_solution << func_name <<", " << best_fitness << endl;
    /*
    if ( (g + 1) % 10 == 0){
        plt::clf();
        plt::xlim(0, 10);
